...
export type ResolveResult =
  ...
  // layers below (position/structural) rather than the recorded locator
  // resolving trivially on its own — the caller (replay.ts) uses it to
  // decide whether there's anything worth healing back onto the source
  ...
  | { ok: true; locator: Locator; healedVia?: "position" | "structural" }
  ...
  // so it doesn't have to re-run the same query.
  | { ok: false; reason: "ambiguous" | "drifted"; detail?: string; candidates: Locator[] };
...
async function pickByRecordedPosition(
  ...
  x: number,
  y: number,
): Promise<{ locator: Locator | null; detail: string }> {
  const distances: { locator: Locator; distance: number }[] = [];
  for (const candidate of candidates) {
    const distance = await distanceFromRecordedPoint(candidate, x, y);
    if (distance !== null) distances.push({ locator: candidate, distance });
  }
  if (distances.length === 0) {
    return { locator: null, detail: "none of the candidates exposed a position to compare" };
  }
  distances.sort((a, b) => a.distance - b.distance);
  const [closest, runnerUp] = distances;
  ...
  return { locator: closest.locator, detail: `${closestPx}px from the recorded click point` };
}
...
export async function resolveStepTarget(page: Page, step: StepResult): Promise<ResolveResult> {
  ...
  // source step.
  let healedVia: "position" | "structural" | undefined;
  if (candidates.length > 1) {
    ...
    if (!picked) return { ok: false, reason: "ambiguous", detail, candidates };
    resolved = picked;
  } else {
  ...
  if (step.locator.strategy === "css" && step.description) {
    ...
    if (tagMatch) {
      ...
      if (actualTag && actualTag !== tagMatch) {
        ...
        if (step.x !== undefined && step.y !== undefined) {
          const distance = await distanceFromRecordedPoint(resolved, step.x, step.y);
          if (distance !== null && distance <= POSITION_MATCH_MAX_DISTANCE) {
            return { ok: true, locator: resolved, healedVia: "position" };
          }
          positionDetail =
            distance === null
              ? "its position couldn't be checked"
              : `it's ${Math.round(distance)}px from the recorded click point (needed ≤${POSITION_MATCH_MAX_DISTANCE}px)`;
        }
